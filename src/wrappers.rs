...
impl ImageWrap {
    pub fn new(inp: PathBuf, shrink: Option<f64>) -> Result<Self, Box<dyn Error>> {
        if let Some(a) = shrink {
    ...
    pub fn save(&self, to: &Path) -> Result<(), Box<dyn Error>> {
...
impl RgbWrap {
    ...
    pub fn to_rgb(inp: &str) -> Result<Self, Box<dyn Error>> {
