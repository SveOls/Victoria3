...
impl<'a> Map {
    pub fn new(inp: &Path) -> Result<Self, VicError> {
        ...
        let mut index_color: Vec<(Option<Rgb<u8>>, usize)> =
            vec![(None, 0); states.iter().map(|x| x.size()).sum::<usize>() + 1];
    ...
    pub fn get_country_color_tag(&self, tag: &str) -> Option<Rgb<u8>> {
        self.countries
            .iter()
            .find(|&x| x.name() == tag)
            .map(|y| y.color().unravel())
    }
    ...
    pub fn state_area(&self) -> impl Iterator<Item = (&StateTemplate, usize)> {
        //-> impl Iterator<Item = (StateTemplate, usize)> {
        self.states.iter().map(|x| {
            (
                x,
                self.index_color
                    .iter()
                    .filter(|y| x.contains(y.0))
                    .fold(0, |a, b| a + b.1),
            )
        })
    }
