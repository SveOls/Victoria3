...
impl Save {
    ...
    pub fn country_cultures(
        &self,
        culture: usize,
    ) -> Result<HashMap<String, (usize, usize)>, VicError> {
        let mut ret = HashMap::new();
        let state_cultures = self.state_cultures(culture)?;
        for (_, country) in &self.countries {
            ret.insert(
                country.tag().to_owned(),
                country
                    .states()
                    .map(|&x| state_cultures[x])
                    .fold((0, 0), |a, b| (b.0 + a.0, b.1 + a.1)),
            );
        }
        Ok(ret)
    }
    ...
    pub fn get_culture(&self, index: usize) -> Result<&Culture, VicError> {
        self.cultures
            .get(&index)
            .ok_or(VicError::Other(Box::new(io::Error::new(
                ErrorKind::Other,
                format!("Couldn't find culture with ID {index} in save"),
            ))))
    }
    pub fn get_state(&self, index: usize) -> Result<&State, VicError> {
        self.states
            ...
            .ok_or(VicError::Other(Box::new(io::Error::new(
                ErrorKind::Other,
                format!("Couldn't find state with ID {index} in save"),
            ))))
    }
    pub fn get_country(&self, index: usize) -> Result<&Country, VicError> {
        self.countries
            ...
            .ok_or(VicError::Other(Box::new(io::Error::new(
                ErrorKind::Other,
                format!("Couldn't find country with ID {index} in save"),
            ))))
    }
}
...
impl GetMapData for Save {
    ...
    fn consume_one(inp: DataStructure) -> Result<Self, VicError> {
        ...
        for i in MapIterator::new(content_outer, DataFormat::Labeled) {
            match i.info() {
                ["pops", content] => {
                    // println!("!!! pops");
                    t_pops = Some(Pop::consume_vec(
                        MapIterator::new(content, DataFormat::Labeled),
                        Some("database"),
                    )?)
                }
                ["states", content] => {
    ...
    fn get_data_from(inp: PathBuf) -> Result<Vec<Self>, VicError> {
        ...
        match zip::ZipArchive::new(std::fs::File::open(&inp)?) {
            ...
            Err(e) => return Err(VicError::Other(Box::new(e))),
        }
